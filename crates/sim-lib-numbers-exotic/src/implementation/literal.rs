//! The `numbers/cf` number-literal shape and class, and helpers exposing them
//! as runtime surface values.
...
impl sim_kernel::ObjectCompat for NumberLiteralClass {
    ...
    fn as_table(&self, cx: &mut Cx) -> Result<Value> {
        let instance_shape = shape_surface_or_symbol(
            cx,
...
impl Callable for NumberLiteralClass {
    fn call(&self, _cx: &mut Cx, _args: Args) -> Result<Value> {
        Err(Error::Eval(format!(
            "class {} does not construct values directly; continued fractions are value-level numbers",
            self.symbol
        )))
    }
}
